...
public class HiveReader {
    ...
    public static class Task extends Reader.Task {
        ...
        public void init() {
            ...
            //获取配置
            this.conf = super.getPluginJobConf();//获取job 分割后的每一个任务单独的配置文件
            this.hiveSql = conf.getString(Key.HIVE_SQL);//获取hive sql
            this.tempDatabase = conf.getString(Key.TEMP_DATABASE,Constant.TEMP_DATABASE_DEFAULT);
            this.tempHdfsLocation = conf.getString(Key.TEMP_DATABASE_HDFS_LOCATION,Constant.TEMP_DATABSE_HDFS_LOCATION_DEFAULT);
            this.hive_cmd = conf.getString(Key.HIVE_CMD,Constant.HIVE_CMD_DEFAULT);
            this.hive_sql_set = conf.getString(Key.HIVE_SQL_SET,Constant.HIVE_SQL_SET_DEFAULT);
            //判断set语句的结尾是否是分号，不是给加一个
            if (!this.hive_sql_set.trim().endsWith(";")) {
            	this.hive_sql_set=this.hive_sql_set + ";";}
            
            this.fieldDelimiter = conf.getString(Key.FIELDDELIMITER,Constant.FIELDDELIMITER_DEFAULT);
            this.hive_fieldDelimiter = this.fieldDelimiter;
                   
            this.fieldDelimiter = StringEscapeUtils.unescapeJava(this.fieldDelimiter);            
            this.conf.set(Key.FIELDDELIMITER, this.fieldDelimiter);//设置hive 存储文件 hdfs默认的分隔符,传输时候会分隔
            
            this.nullFormat=conf.getString(Key.NULL_FORMAT,Constant.NULL_FORMAT_DEFAULT);
            this.conf.set(Key.NULL_FORMAT,this.nullFormat);
            //判断set语句的结尾是否是分号，不是给加一个
            ...
            LOG.info("配置分隔符后:" + this.conf.toJSON());
            this.dfsUtil = new DFSUtil(this.conf);//初始化工具类
        ...
        public void prepare() {
            ...
            String hiveCmd = this.hive_sql_set+" use "+this.tempDatabase+"; create table "
...
//            }
        ...
        public void startRead(RecordSender recordSender) {
            ...
            for (String sourceFile : this.sourceFiles) {
                ...
                UnstructuredStorageReaderUtil.readFromStream(inputStream, sourceFile, this.conf,
...
//            }
        ...
        private String hiveTableName() {
            ...
            FastDateFormat sdf = FastDateFormat.getInstance("yyyyMMddHHmmssSSS");
            String table_Suffix = UUID.randomUUID().toString().replace('-', '_');
            str.append(Constant.TEMP_TABLE_NAME_PREFIX).append(sdf.format(new Date()))
